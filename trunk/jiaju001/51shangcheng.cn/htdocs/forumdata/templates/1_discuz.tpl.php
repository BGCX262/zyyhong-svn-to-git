...
|| checktplrefresh('D:\Website\51shangcheng.cn\htdocs\././templates/default/discuz.htm', 'D:\Website\51shangcheng.cn\htdocs\././templates/default/footer.htm', 1250675105, '1', './templates/default')
|| checktplrefresh('D:\Website\51shangcheng.cn\htdocs\././templates/default/discuz.htm', 'D:\Website\51shangcheng.cn\htdocs\././templates/default/css_script.htm', 1250675105, '1', './templates/default')
|| checktplrefresh('D:\Website\51shangcheng.cn\htdocs\././templates/default/discuz.htm', 'D:\Website\51shangcheng.cn\htdocs\././templates/default/jsmenu.htm', 1250675105, '1', './templates/default')
;?>
...
<? if(!$discuz_uid) { ?>
<p>你可以<a href="<?=$regname?>" onclick="floatwin('open_register', this.href, 600, 400, '600,0');return false;" class="lightlink">注册</a>一个帐号，并以此<a href="logging.php?action=login" onclick="floatwin('open_login', this.href, 600, 400);return false;" class="lightlink">登录</a>，以浏览更多精彩内容，并随时发布观点，与大家交流。</p>
<? } else { ?>
   ...
   <td width="25%">
    <table border="0" cellpadding="0" cellspacing="0" width="100%">
     <tr valign="top">
      ...
      <? if(is_array($new_post_threadlist)) { foreach($new_post_threadlist as $nthread) { ?>       <? if($nthread['replies']) { ?>
        <div style="overflow: hidden;height: 20px;width: 100%;"><a href="viewthread.php?tid=<?=$nthread['tid']?>" <?=$nthread['highlight']?> title='最新帖子 <?="\n"?>所在论坛: <?=$nthread['forumname']?><?="\n"?>主题标题: <?=$nthread['subject']?> <?="\n"?>主题作者: <?=$nthread['author']?><?="\n"?>发表时间: <?=$nthread['date']?><?="\n"?>浏览次数: <?=$nthread['views']?> 次 <?="\n"?>回复次数: <?=$nthread['replies']?> 次<?="\n"?>最后回复: <?=$nthread['lastreplytime']?><?="\n"?>最后发表: <?=$nthread['lastposter']?>'><?=$nthread['view_subject']?></a></div>
                   <? } else { ?>
        <div style="overflow: hidden;height: 20px;width: 100%;"><a href="viewthread.php?tid=<?=$nthread['tid']?>" <?=$nthread['highlight']?> title='最新帖子 <?="\n"?>所在论坛: <?=$nthread['forumname']?><?="\n"?>主题标题: <?=$nthread['subject']?> <?="\n"?>主题作者: <?=$nthread['author']?><?="\n"?>发表时间: <?=$nthread['date']?><?="\n"?>浏览次数: <?=$nthread['views']?> 次 <?="\n"?>回复次数: <?=$nthread['replies']?> 次<?="\n"?>最后回复: 暂时没有回复'><?=$nthread['view_subject']?></a></div>
       <? } ?>
      <? } } ?>      </td>
     </tr>
    </table>
   ...
   <td width="25%">
    <table border="0" cellpadding="0" cellspacing="0" width="100%">
     <tr valign="top">
      <td width="24" style="padding:0;border-top:0px;"><img src="images/list.gif" border="0" /></td>
      <td background="images/listbg.gif" style="line-height:20px;padding:0;border-top:0px;background-repeat: repeat-y" width="100%">
      <? if(is_array($new_reply_threadlist)) { foreach($new_reply_threadlist as $rthread) { ?>       <div style="overflow: hidden;height: 20px;width: 100%;"><a href="viewthread.php?tid=<?=$rthread['tid']?>" <?=$rthread['highlight']?> title='最新回复 <?="\n"?>所在论坛: <?=$rthread['forumname']?><?="\n"?>主题标题: <?=$rthread['subject']?><?="\n"?>主题作者: <?=$rthread['author']?><?="\n"?>发表时间: <?=$rthread['date']?><?="\n"?>浏览次数: <?=$rthread['views']?> 次<?="\n"?>回复次数: <?=$rthread['replies']?> 次<?="\n"?>最后回复: <?=$rthread['lastreplytime']?><?="\n"?>最后发表: <?=$rthread['lastposter']?>'><?=$rthread['view_subject']?></a></div>
      <? } } ?>      </td>
     </tr>
    </table>
   </td>
   <td width="26%">
    <table border="0" cellpadding="0" cellspacing="0" width="100%">
     <tr valign="top">
      <td width="24" style="padding:0;border-top:0px;"><img src="images/list.gif" border="0" /></td>
      <td background="images/listbg.gif" style="line-height:20px;padding:0;border-top:0px;background-repeat: repeat-y" width="100%">
      <? if(is_array($new_hot_threadlist)) { foreach($new_hot_threadlist as $mthread) { ?>       <div style="overflow: hidden;height: 20px;width: 100%;"><a href="viewthread.php?tid=<?=$mthread['tid']?>" <?=$mthread['highlight']?> title='本周热门 <?="\n"?>所在论坛: <?=$mthread['forumname']?><?="\n"?>主题标题: <?=$mthread['subject']?><?="\n"?>主题作者: <?=$mthread['author']?><?="\n"?>发表时间: <?=$mthread['date']?><?="\n"?>浏览次数: <?=$mthread['views']?> 次<?="\n"?>回复次数: <?=$mthread['replies']?> 次<?="\n"?>最后回复: <?=$mthread['lastreplytime']?><?="\n"?>最后发表: <?=$mthread['lastposter']?>'><?=$mthread['view_subject']?></a></div>
      <? } } ?>      </td>
     </tr>
    </table>
   </td>
