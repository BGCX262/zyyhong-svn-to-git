...
|| checktplrefresh('D:\Website\51shangcheng.cn\htdocs\./templates/default/pm.htm', 'D:\Website\51shangcheng.cn\htdocs\./templates/default/seditor.htm', 1250740646, '2', './templates/uchome')
|| checktplrefresh('D:\Website\51shangcheng.cn\htdocs\./templates/default/pm.htm', 'D:\Website\51shangcheng.cn\htdocs\./templates/default/personal_navbar.htm', 1250740646, '2', './templates/uchome')
;?>
<? include template('header'); ?>
<div id="nav"><a href="<?=$indexname?>"><?=$bbname?></a> &raquo; 短消息</div>
...
<a href="pm.php?action=new" onclick="floatwin('open_sendpm', this.href, 600, 410);return false;" class="postpm">+ 发送短消息</a>
<ul>
...
<li <? if($filter == 'systempm') { ?>class="current"<? } ?>><a href="pm.php?filter=systempm" hidefocus="true"><span>系统消息<? if($systemnewpm) { ?>(<?=$systemnewpm?>)<? } ?></span></a></li>
...
<? } ?>
<p>
<span class="time right"><? if($range[$pm['daterange']]) { ?><?=$range[$pm['daterange']]?> <?=$pm['time']?><? } else { ?><?=$pm['date']?> <?=$pm['time']?><? } ?></span>
<cite <? if($pm['new'] && $filter != 'announcepm') { ?>class="new"<? } ?>>
<? if($pm['msgfrom']) { ?><a<? if($pm['msgfromurl']) { ?> href="<?=$pm['msgfromurl']?>"<? } ?> target="_blank"><?=$pm['msgfrom']?></a><? } if($filter != 'privatepm') { ?><?=$pm['subject']?><? } ?>
</cite>
</p>
<div class="summary"><? $pm['message'] = $pm['message'] ? $pm['message'] : '点击这里查看此消息'; if($filter != 'announcepm') { if($pm['touid']) { ?>
<a href="pm.php?uid=<?=$pm['touid']?>&amp;filter=<?=$filter?><? if(!empty($search)) { ?>#pm_<?=$pm['pmid']?><? } else { ?>&amp;daterange=<?=$pm['daterange']?>#new<? } ?>"><?=$pm['message']?></a></div>
<span class="action">
<input name="uid[]" class="checkbox" type="checkbox" value="<?=$pm['touid']?>" />
<a href="pm.php?action=del&amp;uid=<?=$pm['touid']?>&amp;filter=<?=$filter?>" id="pmd_<?=$pm['pmid']?>" onclick="deletepm(this, <?=$pm['pmid']?>);return false;" class="delete">删除</a>
</span>
<? } else { ?>
<a href="pm.php?pmid=<?=$pm['pmid']?>&amp;filter=<?=$filter?>"><?=$pm['message']?></a></div>
<span class="action">
<input name="pmid[]" class="checkbox" type="checkbox" value="<?=$pm['pmid']?>" />
<a href="pm.php?action=del&amp;pmid=<?=$pm['pmid']?>&amp;filter=<?=$filter?>" id="pmd_<?=$pm['pmid']?>" onclick="deletepm(this, <?=$pm['pmid']?>);return false;" class="delete">删除</a>
</span>
<? } } else { ?>
<a href="pm.php?pmid=<?=$pm['pmid']?>&amp;filter=<?=$filter?>"><?=$pm['message']?></a></div>
<? } ?>
...
<span class="pipe">|</span><a href="javascript:;" onclick="doane(event);$('pmform').readopt.value = 1;$('pmform').submit()" class="lightlink">标记已读</a>
<span class="pipe">|</span><a href="javascript:;" onclick="doane(event);$('pmform').readopt.value = 2;$('pmform').submit()" class="lightlink">标记未读</a>
...
<span class="pipe">|</span><a href="pm.php?action=del&amp;uid=<?=$uid?>&amp;filter=<?=$filter?>" onclick="return confirm('确认要清空有短消息记录？');">清空</a>
...
<p>
<span class="time right"><?=$pm['dateline']?></span>
<cite<? if($pm['new']) { ?> class="new"<? } ?>><? if($pm['msgfromid'] != $discuz_uid) { ?><?=$pm['msgfrom']?><? } else { ?><?=$discuz_userss?><? } ?></cite>	
</p>
<div class="summary"><?=$pm['message']?></div>
<span class="action topact">
<a href="pm.php?action=new&amp;pmid=<?=$pm['pmid']?>" onclick="floatwin('open_sendpm', this.href, 600, 410);return false;">转发</a>
</span>
</li>
...
<div class="editor_tb" style="width: 514px"><? $seditor = array('pmreply', array('bold', 'img', 'link', 'quote', 'code', 'smilies')); ?><link href="forumdata/cache/style_<?=STYLEID?>_seditor.css?<?=VERHASH?>" rel="stylesheet" type="text/css" />
<div>
...
<input name="black" type="radio" value="0" onclick="this.form.action.value = 'delblack'"<? if(!$blackall) { ?> checked="checked"<? } ?> /> 接受所有会员消息
<input name="black" type="radio" value="1" onclick="this.form.action.value = 'addblack'"<? if($blackall) { ?> checked="checked"<? } ?> /> 屏蔽所有会员消息
