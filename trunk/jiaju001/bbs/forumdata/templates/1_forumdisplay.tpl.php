...
<div id="foruminfo">
	...
	<div id="nav">
		<p><a id="forumlist" href="<?=$indexname?><? if($areaid==2) { ?>?areaid=2<? } ?>"<? if($forumjump && $jsmenu['1']) { ?> class="dropmenu" onmouseover="showMenu(this.id)"<? } ?>><?=$bbname?></a> <?=$navigation?></p>
...
<li <? if(empty($filter)) { ?> class="current"<? } ?> ><a href="forumdisplay.php?fid=<?=$fid?>">全部</a></li><? if(is_array($forum['threadtypes']['flat'])) { foreach($forum['threadtypes']['flat'] as $id => $name) { if($typeid != $id) { ?>
		<li><a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=type&amp;typeid=<?=$id?>"><?=$name?></a></li><? } else { ?><li class="current"><a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=type&amp;typeid=<?=$id?>"><?=$name?></a></li><? } ?> <? } } ?></ul></div><? } ?>

<div class="mainbox threadlist">
	<div class="headactions"><a href="forumdisplay.php?fid=<?=$fid?>">全部</a></li> <a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=digest">精华</a> 
		<? if($showpoll) { ?> <a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=poll">投票</a><? } ?>
		<? if($showtrade) { ?> <a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=trade">商品</a><? } ?>
		<? if($showreward) { ?> <a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=reward">悬赏</a><? } ?>
		<? if($showactivity) { ?> <a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=activity">活动</a><? } ?>
		<? if($showdebate) { ?> <a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=debate">辩论</a><? } ?>
		<? if($showvideo) { ?> <a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=video">视频</a><? } ?></div>
	<h1>
