<? if(!defined('IN_DISCUZ')) exit('Access Denied'); global $tid, $fid, $thisgid, $catlist, $advitems, $postlist; if(CURSCRIPT == 'forumdisplay' && $queryfloat) { ?>
...
<? if(!empty($advlist['text'])) { ?><div class="ad_text" id="ad_text_none"><table summary="Text Ad" cellpadding="0" cellspacing="1"><?=$advlist['text']?></table></div><? } if(CURSCRIPT == 'index' && !empty($advlist['intercat'])) { if(is_array($catlist)) { foreach($catlist as $key => $cat) { if($advlist['intercat'][$key] = array_merge(($advlist['intercat']['0'] ? $advlist['intercat']['0'] : array()), ($advlist['intercat'][$key] ? $advlist['intercat'][$key] : array()))) { ?>
	...
	<? if(!$thread['special']) { if(is_array($postlist)) { foreach($postlist as $post) { ?>		<? if($post['count'] > 0) { ?>
			<? if(!empty($advlist['thread1'][$post['count']])) { ?><div class="ad_textlink1" id="ad_thread1_<?=$post['count']?>_none"><?=$advlist['thread1'][$post['count']]?></div><? } ?>
			<? if(!empty($advlist['thread2'][$post['count']])) { ?><div class="ad_textlink2" id="ad_thread2_<?=$post['count']?>_none"><?=$advlist['thread2'][$post['count']]?></div><? } ?>
			<? if(!empty($advlist['thread3'][$post['count']])) { ?><div class="ad_pip" id="ad_thread3_<?=$post['count']?>_none"><?=$advlist['thread3'][$post['count']]?></div><? } ?>
		<? } else { ?>
		...
		<? } ?>
	<? } } } ?>
	<div class="ad_column" id="ad_interthread_none"><? if(!empty($advlist['interthread']) && $thread['replies']) { ?><?=$advlist['interthread']?><? } ?></div>
<? } ?>
...
<script type="text/javascript">
	var insenz_status = <? if(!empty($insenz['hardadstatus'])) { ?>1<? } else { ?>0<? } ?>;
...
<? if(!empty($insenz['hardadstatus'])) { ?><script type="text/javascript" src='<?=$insenz['jsurl']?>&gid=<?=$thisgid?>&fid=<?=$fid?>&tid=<?=$tid?>&random=<? echo random(4); ?>' charset="UTF-8"></script><? } ?>
...
<? if(CURSCRIPT == 'viewthread' && $insenz['topicrelatedad'] && !$thread['special']) { ?>
	<script type="text/javascript">var tradekeywords = '<?=$GLOBALS['tradekeywords']?>';</script><? echo stripslashes($insenz['topicrelatedad']); } if(!empty($advlist['float']) || !empty($advlist['couplebanner'])) { ?>
