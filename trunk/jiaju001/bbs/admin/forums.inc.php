...
<?
		...
		$query = $db->query("SELECT f.fid, f.type, f.status, f.name, f.fup, f.displayorder, f.inheritedmod, f.areaid, ff.moderators
     ...
     ORDER BY f.type<>'group', f.displayorder");
		...
		if(is_array($order)) {
			foreach($order as $fid => $value) {
				...
				$db->query("UPDATE {$tablepre}forums SET areaid='$area[$fid]' WHERE fid='$fid'");
			}
		}
